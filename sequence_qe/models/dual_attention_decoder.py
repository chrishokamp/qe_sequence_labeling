"""
A seq2seq decoder for Automatic Post-Editing with attention over both the source sequence and the target MT
"""
...
class DualAttentionDecoderOutput(
    namedtuple("DecoderOutput", [
        "logits", "predicted_ids", "cell_output", "attention_scores_1",
        "attention_context_1", "attention_scores_2", "attention_context_2"
    ])):
    """Augmented decoder output that also includes the attention scores for both encoders
    """
    pass


class DualAttentionDecoder(RNNDecoder):
    """An RNN Decoder that uses attention over two input sequences.
    ...
    Args:
      ...
      attention_keys_1: The sequence used to calculate attention scores for input 1.
