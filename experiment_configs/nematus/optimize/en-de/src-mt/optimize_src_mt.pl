...
my $START_WEIGHTS = "/home/chris/projects/qe_sequence_labeling/experiment_configs/nematus/optimize/en-de/src-mt/run1.dense";
...
#$mosesdecoder/scripts/tokenizer/deescape-special-chars.perl
        execute("python $GBS_DIR/scripts/translate_nematus.py -m $SRC_MODEL_DIR/model.iter370000.npz $MT_MODEL_DIR/model.iter290000.npz -c $SRC_MODEL_DIR/model.iter370000.npz.json $MT_MODEL_DIR/model.iter290000.npz.json -i $DEV_DATA_DIR/dev.src.prepped $DEV_DATA_DIR/dev.mt.prepped --nbest $NBEST --beam_size $BEAM_SIZE --load_weights $WORK/run$i.dense --mert_nbest | sed 's/\@\@ //g' | $MOSES_SCRIPTS/recaser/detruecase.perl | $MOSES_SCRIPTS/tokenizer/deescape-special-chars.perl > $WORK/run$i.out");
...
#        python $GBS_DIR/scripts/translate_nematus.py -m $SRC_MODEL_DIR/model.iter370000.npz $MT_MODEL_DIR/model.iter290000.npz  -c $SRC_MODEL_DIR/model.iter370000.npz.json $MT_MODEL_DIR/model.iter290000.npz.json  -i $DEV_DATA_DIR/dev.src.prepped $DEV_DATA_DIR/dev.mt.prepped --nbest 5 --beam_size 10 --mert_nbest
    ...
    unless(-s "$WORK/run$j.dense") {
        ...
        execute("$MERT --sctype $SCORER --scfile $SCORES --ffile $FEATURES --ifile $WORK/run$i.initopt -d 2 -n 20 -m 20 --threads 20 2> $WORK/mert.run$i.log");
