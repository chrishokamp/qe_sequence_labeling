...
# NOTE: this line IS BROKEN -- may need to also copy `run1.initopt`, or create in another way
dense2init("$WORK/run1.dense", "$WORK/run1.initopt");

execute("rm -rf $WORK/progress.txt");
for my $i (1 .. $MAX_IT) {
    unless(-s "$WORK/run$i.out") {
        execute("python $GBS_DIR/scripts/translate_nematus.py -m $MT_SRC_MODEL_DIR/model.npz $SRC_MODEL_DIR/model.iter370000.npz $MT_MODEL_DIR/model.iter290000.npz -c $MT_SRC_MODEL_DIR/model.npz.json $SRC_MODEL_DIR/model.iter370000.npz.json $MT_MODEL_DIR/model.iter290000.npz.json -i $DEV_DATA_DIR/dev.mt.factor_corpus $DEV_DATA_DIR/dev.src.prepped $DEV_DATA_DIR/dev.mt.prepped --nbest $NBEST --beam_size $BEAM_SIZE --length_factor $LENGTH_FACTOR --load_weights $WORK/run$i.dense --mert_nbest | sed 's/\@\@ //g' | $MOSES_SCRIPTS/recaser/detruecase.perl | $MOSES_SCRIPTS/tokenizer/deescape-special-chars.perl > $WORK/run$i.out");
...
#        execute("$EXTR --sctype QE --reference $DEV_TRG -n $WORK/run$i.out -S $WORK/run$i.scores.dat -F $WORK/run$i.features.dat");
        execute("$EXTR $CONFIG --reference $DEV_MT -n $WORK/run$i.out -S $WORK/run$i.scores.dat -F $WORK/run$i.features.dat");

        # hacky hacks - start
        # Chris: note *12 suffix -- this is nbest list len
        execute("cat $DEV_MT | perl -ne 'print \$_ x 12' > $WORK/dev.mt.12");
        execute("cat $DEV_TAGS | perl -ne 'print \$_ x 12' > $WORK/dev.mt.tags.12");
...
#        execute("cat $WORK/run$i.out | perl -ne 'chomp; \@t = split(/\\|\\|\\|/, \$_); print \"\$t[1]\\n\"' | perl $SCRIPTS/detruecase.perl | $SCRIPTS/deescape-special-chars.perl > $WORK/run$i.out.12");
        ...
        execute("python run_tercom.py -target_file=$WORK/dev.mt.12 -post_edited_file=$WORK/run$i.out.12 2>/dev/null >/dev/null");
        execute("python parse_pra.py -tercom_file=$WORK/run$i.out.12.tercom.out.pra");
        execute("python generate_wmt_submission.py DCU $WORK/dev.mt.12 $WORK/run$i.out.12.tercom.out.pra.tags > $WORK/run$i.out.12.qe.submission");
        execute("python evaluate_wmt15_stats.py $WORK/dev.mt.12 $WORK/dev.mt.tags.12 $WORK/run$i.out.12.qe.submission | perl toScores.pl > $WORK/run$i.scores.dat");
        execute("rm $WORK/run$i.out.12.tercom.*");
        # hacky hacks - end
